...
class TransactionCategorizer:
    ...
    def filter_numerator_transactions(
        ...
        # Deterministic offline keyword matching
        num_lower = (numerator_definition or "").lower()
        matched = []

        if "капитальн" in num_lower or "capex" in num_lower or metric_type == "CAPEX_LIMIT":
            matched = [t for t in transactions if any(w in (t.counterparty.lower() + " " + t.description.lower()) for w in ["оборудован", "строительст", "капитальн", "модернизац", "техник", "кран", "судно", "причал", "монтаж", "реконструкц", "capex", "equipment", "crane"])]
            ...
            matched = [t for t in transactions if any(w in (t.counterparty.lower() + " " + t.description.lower()) for w in ["holding", "управл", "вознагражден", "агентск", "консультац", "роялти", "дивиденд", "aktau", "kaspi", "ural"])]

        if matched:
            return matched

        # Specific category fallback if no keywords matched
        capex_fallback = [t for t in transactions if any(w in t.description.lower() for w in ["crane", "equipment", "capex", "закупка", "строительство"])]
        if capex_fallback:
